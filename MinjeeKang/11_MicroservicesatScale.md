...
>> 대부분의 상황에서는 필요한 것보다 더 많은 컴퓨팅 파워를 보유하는 것이 모자라는 것 보다 훨씬 낫다
***
<br/>
...
#### 주키퍼
- 하둡 프로제긑의 일부로 개발
- 환경 구성 관리, 서비스 간 데이터 동기화, 리더 선출, 메시지 큐, 실용적인 네임이 서비스 등 헤아릴 수 없는 많은 사례에 이용
- 정보르 ㄹ저장하기 위해 계층적 네임스페이스를 사용
- 일반 환경 구성 저장소로 사용되기 때문에 서비스 관련 환경 구성을 저장할 수 있어서 동적으로 로그 수준을 변경하거나 실행 중이 ㄴ시스템의 기능을 끄는 작업을 수행할 수 있다
>> 지금은 구식으로 간주될 수 있고 새로운 대안 솔루션들에 비해 서비스 발견에 도움이 되는 많은 기능을 탑재하지 않았다. 주키퍼가 구현하는 하부 알고리즘은 올바르게 수행하기 어렵다
#### 칸슬
- 환경 구성 관리와 서비스 발견 모두 지원
- 주요 사용 사례에 대한 더 많은 지원을 제공하면서 주키퍼를 앞선다
- 노드의 상태 확인을 수행하느 ㄴ기능처럼 유용한 다른 기능도 구축
- 우수한 장애 허용 설계와 임시 노드를 대량으로 사용하는 시스템을 다루는 데 주력
- 서비스의 등록, 키/값 저장소 질의 또는 상태 확인 삽입 등 모든 작업에 RESTful HTTP 인터페이스를 사용, 따라서 서로 다른 기술 스택과의 통합과 매우 수월
- 칼슬 하부의 서프는 클러스터에서 노드의 발견, 장애 관리, 주의 경보를 처리하고 칸슬은 서비스 발견과 환경 구성 관리를 추가
>> 칸슬은 매우 새롭고 알고리즘이 복잡하므로 중요한 작업에 추천하기가 대체로 망설여진다.
#### 유레카
- 서비스 인스턴스에 대해 기본적인 라운드 로빈 조회를 지원할 수 있다는 점에서 기본적인 부하 분산 기능도 제공
- REST 기반의 엔드포인트를 제공하므로 여러분은 자체 클라이언트나 자바 클라이언트도 작성할 수 있다
- 유레카 자쳉의 클라이언트를 사용하지 않고 REST 엔드포인트로 직접 사용한다면 사용자가 알아서 해야 한다
#### 직접 만들기
1. 한 프로젝트에서 인스턴스에 태그를 추가하는 기능을 제공하는데 AWS를 많이 사용, 서비스 인스턴스를 시작할 때 인스턴스와 용도를 정의하기 위해 태그를 적용
2. 해당 머신을 찾기 위해 주어진 AWS 계정과 연관된 모든 인스턴스에 질의하는 AWS API를 사용할 수 있다
3. 이러한 인스턴스와 통신할 수 있는 명령줄 도구를 구축
#### 사람을 잊지 마라!
...
### 문서화 서비스
마이크로서비스 API 문서는 항상 최신 상태를 유지하고 서비스의 엔드포인트를 알면 쉽게 이 문서를 볼 수 있게 한다
#### 스웨거
스웨거를 사용할 경우 API를 잘 기술하면 웹 브라우저를 통해 API 문서를 볼 수 있고 API와 상호 작용할 수 있는 아주 멋진 웹 UI를 생성할 수 있다. 
#### HAL과 HAL 브라우저
하이퍼텍스트 애플리케이션 언어(HAL) 자체는 우리가 노출하는 하이퍼미디어 컨트롤에 대한 표준을 기술하는 표준이다.<br/>
HAL의 하이퍼미디어 표준을 채택하기로 결정한 경우 API를 사용하기 위해 다양한 클라이언트 라이브러리를 사용할 수 있을 뿐만 아니라 HAL 브라우저를 이용하여 웹 브라우저를 통해 API를 탐색할 수 있다.
***
<br/>

### 자기 기술 시스템
라이브 시스템에서 약간의 데이터를 긁어오는 정적 웹 페이지나 위키와 같은 간단한 것으로 시작하라. 그러나 시간이 지나면서 점점 더 많은 정보의 수집을 고려하라. 이 정보를 쉽게 이요하게 만드는 것은 대규모로 수행되는 시스템에서 발생할 새로운 복잡성을 관리하는 핵심 도구다
***
<br/>
